...
{
	vec3 LightToPixel = normalize(( Light.Direction - Light.Base.Base.Position));
...
void main(void)
   {
	   ...
	   dl[0].Base.AmbientIntensity = .1;
	   dl[0].Base.DiffuseIntensity = .51;
	   dl[0].Base.SpecularIntensity = .8;
	   dl[0].Base.Position = vec3(-2.0,7.0,-6.0);
	   
	   
	   //PointLight
	   pl[0].Base.Color = vec3(1.0,1.0,1.0);
	   pl[0].Base.AmbientIntensity = .05;
	   pl[0].Base.DiffuseIntensity = 2.0;
	   pl[0].Base.SpecularIntensity = 1.0;
	   pl[0].Base.Position = vec3(20.0,light2y,2.0);
	   pl[0].Atten.Constant = 0.0;
	   pl[0].Atten.Linear = .05;
	   pl[0].Atten.Exp = 2.1;
	   
	   //SpotLight
	   sl[0].Base.Base.Color = vec3(0.0,1.0,0.0);
	   sl[0].Base.Base.AmbientIntensity = .00;
	   sl[0].Base.Base.DiffuseIntensity = .15;
	   sl[0].Base.Base.SpecularIntensity = 1000.0;
	   sl[0].Base.Base.Position = vec3(light1x, light1y, light1z);
	   sl[0].Direction = vec3(light1x,light1y ,light1z);
	   sl[0].Base.Atten.Constant = 0.01;
	   sl[0].Base.Atten.Linear = .02;
	   sl[0].Base.Atten.Exp = .8;
	   sl[0].Cutoff = 6.0;
	   
	   //SpotLight2
	   sl[1].Base.Base.Color = vec3(.00,.00,.00);
	   sl[1].Base.Base.AmbientIntensity = .000;
	   sl[1].Base.Base.DiffuseIntensity = .03;
	   sl[1].Base.Base.SpecularIntensity = 100000.0;
	   sl[1].Base.Base.Position = vec3(0.0, 1.2 , 0.0);
	   sl[1].Direction =  vec3(0.0,0.0 ,0.0);
	   sl[1].Base.Atten.Constant = 0.01;
	   sl[1].Base.Atten.Linear = .010;
	   sl[1].Base.Atten.Exp = .1;
	   sl[1].Cutoff = 2.0;

	   
	   vec4 ambient = vec4(dl[0].Base.Color, 1.0) * .2;
