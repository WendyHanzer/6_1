	...
	for (unsigned int i = 0 ; i < pScene->mNumMaterials ; i++)
	   {
		...
		if (!m_Textures[i])
		   {
			m_Textures[i] = new Texture(GL_TEXTURE_2D, "../bin/objects/default.jpg");
...
void Mesh::Render(GLint loc_position, GLint loc_color, GLint loc_normal)
   {
	...
	for (unsigned int i = 0 ; i < m_Entries.size() ; i++)
	   {
		...
		glVertexAttribPointer(loc_color, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (const GLvoid*)(offsetof(Vertex, uv)));
		glVertexAttribPointer(loc_normal, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (const GLvoid*)(offsetof(Vertex, normal)));
